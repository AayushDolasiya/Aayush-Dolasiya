...
import { ArrowDown, Check, Copy, X, Monitor, Database, Code, MapPin, Globe, MessageSquare } from 'lucide-react';
...
export default function Hero({ onViewWork }: HeroProps) {
  ...
  const profileScaleScroll = useTransform(scrollY, [0, 500], [1, 0.97]);

  // Parallax Values
  const pxProfileX = disableParallax ? 0 : useTransform(smoothMouseX, [-0.5, 0.5], [-8, 8]);
  const pxProfileY = disableParallax ? 0 : useTransform(smoothMouseY, [-0.5, 0.5], [-8, 8]);
  
  const pxCircleX = disableParallax ? 0 : useTransform(smoothMouseX, [-0.5, 0.5], [-2, 2]);
  const pxCircleY = disableParallax ? 0 : useTransform(smoothMouseY, [-0.5, 0.5], [-2, 2]);

  const pxDotsX = disableParallax ? 0 : useTransform(smoothMouseX, [-0.5, 0.5], [-6, 6]);
  const pxDotsY = disableParallax ? 0 : useTransform(smoothMouseY, [-0.5, 0.5], [-6, 6]);

  const pxCardsX = disableParallax ? 0 : useTransform(smoothMouseX, [-0.5, 0.5], [-3, 3]);
  const pxCardsY = disableParallax ? 0 : useTransform(smoothMouseY, [-0.5, 0.5], [-3, 3]);

  const handleMouseMove = (e: React.MouseEvent) => {
    if (window.innerWidth < 1024 || disableParallax) return;
...
=====================================================`;
  ...
  return (
    <section 
      ...
      className="bg-brand-yellow dark:bg-primary-container w-full min-h-[90vh] flex flex-col justify-center pt-24 pb-16 px-4 md:px-8 lg:px-16 border-b-4 border-brand-dark dark:border-brand-light transition-colors relative overflow-hidden"
    >
      <motion.div 
        ...
        initial="hidden"
        animate="visible"
        className="w-full max-w-[1400px] mx-auto grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 items-center relative"
      >

        {/* MOBILE ONLY HEADER */}
        <motion.div variants={itemVariants} className="xl:hidden flex flex-col gap-4 md:gap-6 z-10 order-1">
          <div className="inline-flex items-center gap-3 bg-surface-container-lowest dark:bg-brand-dark px-5 py-2.5 rounded-full w-max border-2 border-brand-dark dark:border-brand-light shadow-[4px_4px_0px_0px_#161d18] dark:shadow-[4px_4px_0px_0px_#ebf3ea]">
            <motion.div 
              animate={{ opacity: [1, 0.4, 1], scale: [1, 1.2, 1] }} 
              transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
              className="w-3.5 h-3.5 bg-emerald-500 rounded-full"
            />
          ...
          <div className="flex flex-col gap-1 md:gap-2">
            ...
            <div className="overflow-hidden">
              <motion.h1 variants={textRevealVariants} className="font-headline text-6xl md:text-7xl lg:text-7xl text-brand-dark dark:text-inverse-surface uppercase font-black leading-none tracking-tighter">
                AAYUSH.
              </motion.h1>
            </div>
          </div>
        ...
        <div className="xl:col-span-4 flex flex-col gap-4 md:gap-6 z-10 order-3 xl:order-1">
          ...
          <div className="hidden xl:flex flex-col gap-4 md:gap-6">
            <motion.div variants={itemVariants} className="inline-flex items-center gap-3 bg-surface-container-lowest dark:bg-brand-dark px-5 py-2.5 rounded-full w-max border-2 border-brand-dark dark:border-brand-light shadow-[4px_4px_0px_0px_#161d18] dark:shadow-[4px_4px_0px_0px_#ebf3ea]">
              <motion.div 
                animate={{ opacity: [1, 0.4, 1], scale: [1, 1.2, 1] }} 
                transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                className="w-3.5 h-3.5 bg-emerald-500 rounded-full"
            ...
            <motion.div variants={itemVariants} className="flex flex-col gap-1 md:gap-2">
              <span className="font-mono text-lg md:text-xl font-bold text-brand-dark/70 dark:text-inverse-surface/70 tracking-wider">
                I'M
              </span>
              <div className="overflow-hidden pb-2">
                <motion.h1 
                  ...
                  whileHover={{ y: -2 }}
                  className="font-headline text-6xl md:text-7xl lg:text-7xl xl:text-[90px] text-brand-dark dark:text-inverse-surface uppercase font-black leading-none tracking-tighter cursor-default"
                >
                  AAYUSH.
                </motion.h1>
              </div>
          ...
          <motion.p variants={itemVariants} className="font-sans text-base md:text-lg text-brand-dark/80 dark:text-inverse-surface/90 font-medium leading-relaxed max-w-md">
          ...
          {/* Buttons */}
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row flex-wrap gap-4 xl:gap-3 2xl:gap-4 mt-2">
            <motion.button
              whileHover={{ y: -2 }}
              whileTap={{ y: 1 }}
              onClick={() => setShowResumeModal(true)}
              className="group neo-btn w-full sm:w-auto bg-brand-dark dark:bg-inverse-surface text-brand-light dark:text-inverse-on-surface px-6 md:px-8 xl:px-5 2xl:px-8 py-3.5 rounded-full border-2 border-brand-dark dark:border-brand-light shadow-[4px_4px_0px_0px_rgba(22,29,24,1)] hover:shadow-[3px_3px_0px_0px_rgba(22,29,24,1)] cursor-pointer whitespace-nowrap"
            >
              <span className="flex items-center justify-center gap-2 text-sm md:text-base xl:text-sm 2xl:text-base">
                <motion.span variants={{ hover: { x: 3, y: 3 } }} inherit={false} whileHover="hover" transition={{ type: "spring", stiffness: 400, damping: 25 }} className="flex items-center group-hover:translate-x-[2px] group-hover:translate-y-[2px] transition-transform">
                  <ArrowDown size={18} />
                </motion.span>
                DOWNLOAD CV
              </span>
            ...
            <motion.button
              ...
              className="group neo-btn w-full sm:w-auto bg-transparent text-brand-dark dark:text-brand-light px-6 md:px-8 xl:px-5 2xl:px-8 py-3.5 rounded-full border-2 border-brand-dark dark:border-brand-light shadow-[4px_4px_0px_0px_rgba(22,29,24,1)] hover:shadow-[3px_3px_0px_0px_rgba(22,29,24,1)] cursor-pointer whitespace-nowrap"
            >
              <span className="flex items-center justify-center gap-2 text-sm md:text-base xl:text-sm 2xl:text-base">
                <motion.span className="flex items-center group-hover:translate-x-[2px] transition-transform">
        ...
        <div className="xl:col-span-4 relative flex justify-center items-center w-full min-h-[350px] md:min-h-[450px] xl:min-h-[500px] order-2 xl:order-2 z-0">
          {/* Dotted patterns */}
          <motion.div style={{ x: pxDotsX, y: pxDotsY }} className="absolute top-6 right-0 w-32 h-32 opacity-20 bg-[radial-gradient(#161d18_2px,transparent_2px)] [background-size:16px_16px] dark:bg-[radial-gradient(#f4fbf3_2px,transparent_2px)] z-0" />
          <motion.div style={{ x: pxDotsX, y: pxDotsY }} className="absolute bottom-6 left-0 w-32 h-32 opacity-20 bg-[radial-gradient(#161d18_2px,transparent_2px)] [background-size:16px_16px] dark:bg-[radial-gradient(#f4fbf3_2px,transparent_2px)] z-0" />

          {/* Dark Circle */}
          <motion.div 
            style={{ x: pxCircleX, y: pxCircleY }}
            animate={disableParallax ? {} : { scale: [1, 1.02, 1] }}
            transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }}
            className="absolute w-[280px] h-[280px] md:w-[350px] md:h-[350px] lg:w-[400px] lg:h-[400px] bg-brand-dark/10 dark:bg-brand-light/10 rounded-full z-10 mx-auto" 
          />
          ...
          <motion.img
            ...
            animate={{ 
              opacity: 1, 
              y: disableParallax ? 0 : [0, -6, 0], 
              scale: 1 
            }}
            transition={{ 
              opacity: { duration: 0.5, ease: "easeOut" },
              y: disableParallax ? { duration: 0.6, ease: "easeOut" } : { 
                duration: 4.5, 
                repeat: Infinity, 
                ease: "easeInOut",
                delay: 0.6 // Wait for entrance to finish
              },
              scale: { duration: 0.6, ease: "easeOut" }
            }}
            style={{ x: pxProfileX, y: pxProfileY, scale: profileScaleScroll }}
            src="/hero-image.png"
            alt="Aayush"
        ...
        <motion.div 
          style={{ x: pxCardsX, y: pxCardsY }}
          className="xl:col-span-4 flex flex-col gap-4 w-full z-10 order-4 xl:order-3 md:max-w-md md:mx-auto xl:mx-0 xl:max-w-none"
        >
          {/* What I Do Card */}
          <motion.div variants={whatIDoContainer} className="bg-brand-yellow/30 dark:bg-brand-dark/30 backdrop-blur-sm p-5 md:p-6 rounded-3xl border-2 border-brand-dark/20 dark:border-brand-light/20 shadow-xl w-full">
            ...
            <div className="flex flex-col gap-4">
              <motion.div variants={whatIDoRow} className="flex gap-4 items-start">
                <motion.div whileHover={{ scale: 1.06, rotate: 2 }} transition={{ type: "spring", stiffness: 400, damping: 20 }} className="w-9 h-9 rounded-full bg-brand-dark dark:bg-brand-light flex items-center justify-center text-brand-yellow dark:text-brand-dark flex-shrink-0 shadow-[2px_2px_0px_0px_rgba(22,29,24,0.3)] cursor-default">
                  <Monitor size={18} />
                </motion.div>
          ...
          {/* Location Card */}
          <motion.div variants={itemVariants} className="bg-brand-yellow/30 dark:bg-brand-dark/30 backdrop-blur-sm p-5 rounded-2xl border-2 border-brand-dark/20 dark:border-brand-light/20 shadow-lg relative overflow-hidden flex justify-between items-center w-full group hover:border-brand-dark/40 transition-colors">
